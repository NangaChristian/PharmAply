    ...
    uploadTask.on('state_changed', 
      ...
      async () => {
        try {
           ...
           // Note: In an ideal scenario, maybe trigger a re-render or auth state refresh, but updateProfile updates the current user object.
           toast.success("Profile photo updated! Please refresh the page if not instantly visible.");
        } catch (error) {
           console.error(error);
  ...
  return (
    <div className="flex-1 bg-slate-50 flex flex-col h-full overflow-hidden">
      <div className="bg-white px-8 pt-6 pb-6 shadow-sm z-10 border-b border-gray-200 shrink-0">
         <h1 className="font-bold text-gray-900 text-2xl mb-1">Profile Settings</h1>
         <p className="text-gray-500 text-sm">Manage your administrator account</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 max-w-4xl space-y-6">
         ...
         <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
            <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
               <Key size={18} className="text-slate-400" /> Password & Security
            </h3>
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-between">
               <div>
                  <p className="font-bold text-slate-800 text-sm">Update Password</p>
