...
export function PatientCheckout() {
    ...
    if (e.target.files && e.target.files[0] && user) {
      ...
      try {
        ...
        try {
          ...
          console.warn("Upload failed, using placeholder url:", storageErr);
          if (storageErr?.code === 'storage/quota-exceeded') {
             alert(t('storage_quota_exceeded', "Storage Quota Exceeded. Using a placeholder image for now."));
          }
          url = `https://via.placeholder.com/800x1200.png?text=${encodeURIComponent(file.name)}`;
        }
  ...
  if (loading) return <div className="p-8 text-center text-sm text-gray-500">{t('loading_checkout', 'Loading checkout...')}</div>;
  ...
  return (
    <div className="flex-1 bg-slate-50 flex flex-col h-full overflow-hidden relative">
      <div className="px-6 pt-12 pb-4 flex items-center justify-between bg-white shadow-sm z-10">
         ...
         <h1 className="font-bold text-gray-900 text-sm">{t('checkout', 'Checkout')}</h1>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
         ...
         <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm space-y-4">
            <h3 className="font-bold text-gray-900 border-b border-gray-50 pb-3">{t('order_summary', 'Order Summary')}</h3>
            ...
            <div className="flex gap-4">
               ...
               <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500 mt-1">{t(product.category?.replace(/\s+/g, '_').toLowerCase(), product.category)}</p>
         ...
         {requiresPrescription && (
             <div>
                <div className="flex items-center gap-2 mb-3 px-1">
                   <h3 className="font-bold text-gray-900 text-sm">{t('prescription_validation', 'Prescription Validation')}</h3>
                ...
                {!prescriptionUrl ? (
                    ...
                    >
                       <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm text-orange-500 mb-3">
         ...
         {/* Delivery Address */}
         <div>
            <h3 className="font-bold text-gray-900 mb-3 text-sm px-1">{t('delivery_address', 'Delivery Address')}</h3>
            <div className="bg-white border border-gray-100 rounded-2xl p-4 flex items-center justify-between cursor-pointer">
               <div className="flex items-center gap-4">
                  <div className="w-10 h-10 bg-indigo-50 rounded-full flex items-center justify-center text-indigo-600">
                     <MapPin size={20} />
                  </div>
                  <div>
                     <p className="font-bold text-gray-900 text-sm">{t('home', 'Home')}</p>
                     <p className="text-xs text-gray-500 mt-0.5">Douala, Akwa St.</p>
                  </div>
               </div>
         ...
         {/* Payment Method */}
         <div>
            <h3 className="font-bold text-gray-900 mb-3 text-sm px-1">{t('payment_method', 'Payment Method')}</h3>
            <div 
              onClick={() => setShowPaymentSelector(!showPaymentSelector)}
              className="bg-white border border-gray-100 rounded-2xl p-4 flex items-center justify-between cursor-pointer"
            >
               <div className="flex items-center gap-4">
                  ...
                  <div>
                     <p className="font-bold text-gray-900 text-sm">{paymentMethod || t('select_payment_method', 'Select Payment Method')}</p>
                     <p className="text-xs text-gray-500 mt-0.5">{t('tap_to_change', 'Tap to change')}</p>
                  </div>
               </div>
               <ChevronRight size={20} className={`text-gray-400 transition transform ${showPaymentSelector ? 'rotate-90' : ''}`} />
            </div>
            
            {showPaymentSelector && (
              <div className="mt-3 bg-white border border-gray-100 rounded-2xl p-2 space-y-1 shadow-sm">
                <button onClick={() => { setPaymentMethod("MTN Mobile Money"); setShowPaymentSelector(false); }} className={`w-full flex items-center justify-between p-3 rounded-xl border-2 transition ${paymentMethod === "MTN Mobile Money" ? 'border-yellow-400 bg-yellow-50/30' : 'border-transparent hover:bg-gray-50'}`}>
                   <div className="flex items-center gap-3">
                      <div className="w-8 h-8 bg-yellow-400 text-gray-900 rounded-full flex items-center justify-center font-bold text-xs">MTN</div>
                      <span className="font-bold text-sm text-gray-900">MTN Mobile Money</span>
                   </div>
                ...
                <button onClick={() => { setPaymentMethod("Orange Money"); setShowPaymentSelector(false); }} className={`w-full flex items-center justify-between p-3 rounded-xl border-2 transition ${paymentMethod === "Orange Money" ? 'border-orange-500 bg-orange-50/30' : 'border-transparent hover:bg-gray-50'}`}>
                   <div className="flex items-center gap-3">
                ...
                <button onClick={() => { setPaymentMethod("Cash on Delivery"); setShowPaymentSelector(false); }} className={`w-full flex items-center justify-between p-3 rounded-xl border-2 transition ${paymentMethod === "Cash on Delivery" ? 'border-green-500 bg-green-50/30' : 'border-transparent hover:bg-gray-50'}`}>
                   <div className="flex items-center gap-3">
                ...
                <button onClick={() => { setPaymentMethod("Credit Card"); setShowPaymentSelector(false); }} className={`w-full flex items-center justify-between p-3 rounded-xl border-2 transition ${paymentMethod === "Credit Card" ? 'border-blue-500 bg-blue-50/30' : 'border-transparent hover:bg-gray-50'}`}>
                   <div className="flex items-center gap-3">
         ...
         <div className="bg-white border border-gray-100 rounded-2xl p-5 space-y-3">
            ...
            <div className="flex justify-between text-sm">
               <span className="text-gray-500">{t('delivery_fee', 'Delivery Fee')}</span>
               <span className="font-bold text-gray-900">{formatCurrency(1000)}</span>
            </div>
            <div className="h-px bg-gray-100 my-2"></div>
            <div className="flex justify-between">
               <span className="font-bold text-gray-900 text-lg">{t('total', 'Total')}</span>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 px-6 pb-8 z-20">
         <button 
