...
export function PatientOrders() {
  ...
  return (
    <div className="flex-1 bg-slate-50 flex flex-col h-full overflow-hidden">
      <div className="bg-white px-6 pt-12 pb-4 shadow-sm z-10 flex flex-col gap-4">
         <h1 className="font-bold text-gray-900 text-xl">{t('orders', 'Orders')}</h1>
         
         <div className="relative">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
            <input type="text" placeholder={t('search_orders', 'Search orders...')} className="w-full bg-gray-100 py-3 pl-12 pr-4 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-100" />
         </div>
         
         <div className="flex gap-4 border-b border-gray-100 pb-2 text-sm font-medium">
            <button className="text-indigo-600 border-b-2 border-indigo-600 pb-2">{t('active', 'Active')} ({activeOrders.length})</button>
            <button className="text-gray-400 pb-2">{t('past', 'Past')} ({pastOrders.length})</button>
         </div>
      </div>

      <div className="flex-1 overflow-y-auto p-6 space-y-6">
         {loading ? <p className="text-center text-sm text-gray-500 py-4">{t('loading_orders', 'Loading orders...')}</p> : (
           <>
             {/* Active Orders */}
             <div className="space-y-4">
                <h3 className="font-bold text-gray-900 text-sm">{t('active_orders', 'Active Orders')}</h3>
                {activeOrders.map(order => (
                   <div key={order.id} className={`bg-white p-4 rounded-2xl border shadow-sm ${order.status === 'substitution_proposed' ? 'border-indigo-200 shadow-indigo-100/50' : 'border-orange-200 shadow-orange-100/50'}`}>
                      <div className="flex justify-between items-start mb-3 border-b border-gray-50 pb-3">
                         <div className="flex items-center gap-3">
                            ...
                            <div>
                               <p className="font-bold text-gray-900 text-sm">{t('order', 'Order')} #{order.id.slice(0,8)}</p>
                      ...
                      {order.status === 'substitution_proposed' && (
                         <div className="mb-4 bg-indigo-50 p-3 rounded-xl border border-indigo-100">
                            ...
                            <div className="flex gap-2">
                               <button onClick={() => {}} className="flex-1 py-1.5 bg-white border border-red-100 text-red-600 text-xs font-bold rounded-lg flex items-center justify-center gap-1">
                      ...
                      <div className="flex justify-between items-center text-xs text-gray-500 font-medium">
                         <span>{order.createdAt?.toDate().toLocaleDateString() || t('recently', 'Recently')}</span>
                      ...
                      {cancellingOrder === order.id && (
                        <div className="mt-4 pt-3 border-t border-gray-100">
                          ...
                          <select 
                            className="w-full border border-gray-200 p-2 rounded-lg bg-gray-50 text-xs mb-2" 
                            value={cancelReason} 
                          ...
                          <div className="flex gap-2 mt-2">
                             <button onClick={() => setCancellingOrder(null)} className="flex-1 py-1.5 bg-gray-100 text-gray-700 rounded-lg text-xs font-bold transition hover:bg-gray-200">{t('keep_order', 'Keep Order')}</button>
                ...
                {activeOrders.length === 0 && (
                   <div className="text-center py-10 px-6 bg-white rounded-2xl border border-gray-100 border-dashed flex flex-col items-center justify-center">
                     <div className="w-16 h-16 bg-gray-50 text-gray-300 rounded-full flex items-center justify-center mb-4">
                        <Activity size={32} />
                     </div>
                     <h3 className="font-bold text-gray-900 mb-1">{t('no_active_orders', 'No active orders')}</h3>
                     <p className="text-sm text-gray-500 mb-6">{t('no_active_orders_desc', "You don't have any active orders right now. Start shopping to see them here.")}</p>
             ...
             {/* Past Orders */}
             <div className="space-y-4">
                <h3 className="font-bold text-gray-900 text-sm">{t('past_orders', 'Past Orders')}</h3>
                {pastOrders.map(order => (
                   <div key={order.id} className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
                      ...
                      <div className="flex justify-between items-center text-xs text-gray-500 font-medium">
                         <span>{order.createdAt?.toDate().toLocaleDateString() || t('unknown_date', 'unknown date')}</span>
                         <span>{formatCurrency(order.total)}</span>
                      </div>
