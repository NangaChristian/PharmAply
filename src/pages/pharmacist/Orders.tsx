...
export function PharmacistOrders() {
  ...
  // Extract patient IDs to subscribe in real-time
  ...
  const filteredOrders = useMemo(() => {
    return sortByDateDesc(
      orders.filter(o => {
        ...
        const patientId = o.patientId || o.userId;
        const livePatientName = userProfiles[patientId]?.name || o.patientName || '';
        const matchSearch = !searchTerm.trim() || 
          o.id?.toLowerCase().includes(searchTerm.toLowerCase()) ||
          livePatientName.toLowerCase().includes(searchTerm.toLowerCase()) ||
          o.items?.some((i: any) => i.name?.toLowerCase().includes(searchTerm.toLowerCase()));
        return matchTab && matchSearch;
      })
    );
  }, [orders, activeTab, searchTerm, userProfiles]);
      ...
      <div className="px-8 py-6 flex items-center justify-between shrink-0">
          <div className="flex-1 flex items-center">
             <div className="relative w-full max-w-sm">
                ...
                <input 
                   type="text" 
                   placeholder={t('search_orders', 'Rechercher des commandes...')} 
                   value={searchTerm}
               ...
               <table className="w-full text-left border-collapse">
                  <thead>
                     <tr className="border-b border-gray-100 dark:border-slate-700">
                        ...
                        <th className="py-4 px-6 text-xs font-bold tracking-wider text-gray-500 uppercase text-right">{t('actions', 'Action')}</th>
                     </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50 dark:divide-slate-700/50">
                     {loading ? (
                        <tr><td colSpan={7} className="py-8 text-center text-gray-500 text-sm animate-pulse">{t('loading_orders', 'Chargement des commandes...')}</td></tr>
                     ) : filteredOrders.length === 0 ? (
                        <tr><td colSpan={7} className="py-8 text-center text-gray-500 text-sm">{t('no_active_orders', 'Aucune commande.')}</td></tr>
                     ) : (
